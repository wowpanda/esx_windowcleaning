...
function StartNPCJob()
    ...
    Blips['NPCTargetWindows'] = AddBlipForCoord(zone.Pos.x,  zone.Pos.y,  zone.Pos.z)
    SetBlipRoute(Blips['NPCTargetWindows'], true)
...
function StopNPCJob(cancel)
    ...
    if Blips['NPCTargetWindows'] ~= nil then
      RemoveBlip(Blips['NPCTargetWindows'])
      Blips['NPCTargetWindows'] = nil
	end
...
AddEventHandler('esx_windowclener:hasEnteredMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, 316) and PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
			...
			if Onjob then
				...
				if IsPedInAnyVehicle(playerPed,  false) and IsVehicleModel(GetVehiclePedIsIn(playerPed,  false), GetHashKey("freecrawler")) then
					StartNPCJob()
					Onjob = true
				else
