...
RegisterServerEvent('esx_windowcleaning:GiveItem')
AddEventHandler('esx_windowcleaning:GiveItem', function()
	...
	if Quantity >= 20 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'You have cleaned ~g~x' .. amount .. ' windows')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, '~r~You n\'have more than '..Config.Zones.Vente.ItemRequires_name..' to cash.')
				...
				TriggerClientEvent('esx:showNotification', _source, 'You have received ~g~$' .. Config.Zones.Vente.ItemPrice)
				Vente(_source)
			end
...
RegisterServerEvent('esx_windowcleaning:startVente')
AddEventHandler('esx_windowcleaning:startVente', function()
	...
	if PlayersVente[_source] == false then
		...
		TriggerClientEvent('esx:showNotification', _source, '~g~Action ~w~in progres...')
		Vente(_source)
	end
end)

RegisterServerEvent('esx_windowcleaning:stopVente')
AddEventHandler('esx_windowcleaning:stopVente', function()
